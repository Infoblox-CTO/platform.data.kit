# CloudQuery Go Types Contract

## contracts/cloudquery.go (new file)
...
## contracts/datapackage.go (modified — add CloudQuery field)

```go
// Add to DataPackageSpec struct:
type DataPackageSpec struct {
	// ... existing fields ...
	CloudQuery *CloudQuerySpec `yaml:"cloudquery,omitempty" json:"cloudquery,omitempty"`
}
```

## contracts/types.go (modified — add PackageType constant)

```go
const PackageTypeCloudQuery PackageType = "cloudquery"
```
