...
- Go 1.25 (multi-module monorepo: `cli/`, `sdk/`, `contracts/`) + `gopkg.in/yaml.v3` (parsing), `github.com/santhosh-tekuri/jsonschema/v6` (JSON Schema validation), `oras.land/oras-go/v2` (OCI registry), `github.com/spf13/cobra` (CLI) (011-dataset-contracts)
- Local filesystem (`dataset/` directory tree); OCI registry for schema resolution (011-dataset-contracts)
...
- Filesystem — `dk.yaml` as YAML files in project root, datasets under `datasets/` directory (012-pipeline-workflows)
